...
export default function FormReceta({
  ...
  return (
    <div className="mt-2 p-4 rounded-2xl ring-1 ring-blue-200 bg-blue-50/60 font-sans text-base">
      {/* AÑADIDO: font-sans y un tamaño base opcional */}
      ...
      <div className="mt-3 space-y-3">
        {meds.map((m, i) => (
          <div key={i} className="bg-white ring-1 ring-slate-200 rounded-xl p-3 grid grid-cols-1 md:grid-cols-12 gap-2">
            <input
              ...
              className="md:col-span-4 border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300 border-slate-300 w-full"
            />
            <input
              value={m.dosis}
            ...
            <div className="md:col-span-4 flex gap-2">
              <input
                ...
                className="flex-1 border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300 border-slate-300 w-full"
              />
              {meds.length > 1 && (
                <button
                  onClick={() => removeMed(i)}
        ...
        <div className="pt-1">
          <button
            onClick={addMed}
            className="w-full md:w-auto px-3 py-2 rounded-md bg-white text-blue-700 ring-1 ring-blue-200 hover:bg-blue-50"
          >
            + Añadir medicamento
          </button>
        </div>
